...
contract(
  "GEM",
  ([deployer, governance, whitelistedUser, user1, user2, user3]) => {
    ...
    before(async () => {
      ...
      token.grantRole(WHITELISTED_ROLE, whitelistedUser, { from: governance });

      token.transfer(whitelistedUser, web3.utils.toWei("1000", "ether"), {
        from: governance,
      });
    ...
    it("should allow transferFrom for whitelisted user", async () => {
      ...
      let res = await token.transferFrom(user1, user2, amount, {
    ...
    it("should allow not whitelisted user to transfer from whitelisted", async () => {
      ...
      let res = await token.transferFrom(whitelistedUser, user2, amount, {
        from: user1,
      });
      expectEvent(res, "Transfer");
    });
    ...
    it("should be able to return balance at snapshot", async () => {
      ...
      expect(after.user1balance).to.be.bignumber.equal(
        before.user1balance.add(amount)
      );
      expect(after.user2balance).to.be.bignumber.equal(
        before.user2balance.sub(amount)
      );
      expect(after.user1balanceSnapshot).to.be.bignumber.equal(
        before.user1balance
      );
      expect(after.user2balanceSnapshot).to.be.bignumber.equal(
        before.user2balance
      );
    });
  }
